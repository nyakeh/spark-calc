  ...
  return (
    <div>
      <section className="App-main">
        ...
        <p>
          You"re on track to have <span>{Pound.format(retirementNetWorth)}</span> at retirement. You can take{" "}
          <span>{Pound.format(safeWithdrawalAmount)}</span> per year.
        </p>
        <p>
          To coast today you"d need <span>{Pound.format(todayCoastFigure)}</span> invested
        </p>
        <p>
          Your initial <span>{Pound.format(initialAssets)}</span> will grow to{" "}
          <span>{Pound.format(initialSumCompounded)}</span> over the <span>{yearsToRetirement}</span> years till
          retirement age
        </p>
      </section>
